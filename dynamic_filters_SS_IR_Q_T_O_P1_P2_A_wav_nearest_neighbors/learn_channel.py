...
for array in marco_data.keys():
    for ss in['single_sources-75d', 'single_sources-15d', 'single_sources-30d', 'organ', 'acapella', 'single_sources-90d', 'piano_solo_1', 'single_sources-45d', 'single_sources0deg', 'single_sources-60d', 'trio', 'quartet']: 
...
K = 33
ch1 = nchans*1
F1 = tf.Variable(tf.initializers.HeNormal()(shape=(1,K,1,ch1)))
...
# capsule coordinates network parameters
oW1 = tf.Variable(tf.initializers.HeNormal()(shape=(3,sH1)))
ob1 = tf.Variable(tf.constant_initializer(0.0)(shape=(sH1,)))
oW2 = tf.Variable(tf.initializers.HeNormal()(shape=(sH1,sH2)))
ob2 = tf.Variable(tf.constant_initializer(0.0)(shape=(sH2,)))
oW3 = tf.Variable(tf.initializers.HeNormal()(shape=(sH2,sH3)))
ob3 = tf.Variable(tf.constant_initializer(0.0)(shape=(sH3,)))
        ...
        for ss in marco_data[array].keys():
            ...
            for in_size in [3]:#reversed(range(nchans-1,nchans)):
                ...
                if patience_count >= patience:
                    ...
                    plt.colorbar()
                    if epoch == final_epoch:
                        plt.savefig('_'.join(['plots/train_fft_error',ss,array,str(in_size),'.png']))
                    if epoch == final_epoch+1:
                        plt.savefig('_'.join(['plots/eval_fft_error',ss,array,str(in_size),'.png']))
                    plt.close()
