...
from rave.udls import SimpleDataset, simple_audio_preprocess
from rave.udls.transforms import Compose, RandomApply, Dequantize, RandomCrop
...
if __name__ == "__main__":
    ...
    class args(Config):
        ...
        MICARRAY_SHAPES_FILE = '/home/iran/virtualarray/micarrays/eigenmike/tetras_dict.pkl'
    ...
    model = RAVE(
        ...
        latent_size=args.LATENT_SIZE, # this is somehow "learned"?
        ratios=args.RATIOS,
        bias=args.BIAS,
    ...
    # validate the model
    x = 0.9*torch.ones(args.BATCH, 31, 2**14)
    xc = 0.9*torch.ones(args.BATCH, 31, 3)
    x = [x,xc]
    y = 0.9*torch.ones(args.BATCH, 2**14)
    yc = 0.9*torch.ones(args.BATCH, 3)
    y = [y, yc]
    out_val = model.validation_step((x,y),0)
    ...
    dataset = SimpleDataset(
        ...
        micarray_shapes_file=args.MICARRAY_SHAPES_FILE
    )

    val = max(int(0.5 * len(dataset)) // 100, 1)
