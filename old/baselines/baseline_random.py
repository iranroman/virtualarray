...
for array in marco_data.keys():
    ...
    for ss in marco_data[array].keys():
        ...
        for ichan in range(nchans):
            ...
            mus = np.mean(Y, axis=2, keepdims=True)
            sigma = np.sqrt(np.mean(np.square(Y - mus)))
            out = sigma*np.random.randn(*Y.shape) + mus
            error = mse(out,Y)
            total_error += error 
        ...
        axs[2].grid(True)
        plt.savefig('_'.join(['plots/eval_inference_normal',ss,array,'.png']))
        plt.close()
        ...
        plt.colorbar()
        plt.savefig('_'.join(['plots/eval_inference_normal_fft_error',ss,array,'.png']))
        plt.close()

    print('-----------------------------')
    print('                             ')
    print('For array',array,'the average error was', total_error.numpy()/nchans)
    print('                             ')
    print('-----------------------------')
