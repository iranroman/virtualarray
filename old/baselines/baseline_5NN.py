...
for array in marco_data.keys():
        ...
        for in_size in [1]:
            ...
            for ichan in range(nchans):
                ...
                for chans_in in all_chans_in:
                    ...
                    distances.append(np.linalg.norm(Xc[0]-Yc))
                    distances.append(np.linalg.norm(Xc[1]-Yc))
                    distances.append(np.linalg.norm(Xc[2]-Yc))
                    distances.append(np.linalg.norm(Xc[3]-Yc))
                    distances.append(np.linalg.norm(Xc[4]-Yc))
                    angles.append(angle_between(Xc[0], Yc))
                    angles.append(angle_between(Xc[1], Yc))
                    angles.append(angle_between(Xc[2], Yc))
                    angles.append(angle_between(Xc[3], Yc))
                    angles.append(angle_between(Xc[4], Yc))
    ...
    print('-----------------------------')
    print('                             ')
    print('For array', array,', the average error was', total_error/nchans)
