...
for array in marco_data.keys():
    ...
    for ss in marco_data[array].keys():
        ...
        for ichan in range(nchans):
            ...
            all_chans_in = range(nchans)
            all_chans_in = [[c] for c in all_chans_in]
            ...
            for chans_in in all_chans_in:
                ...
                distance = np.linalg.norm(Xc-Yc)
                angle = angle_between(Xc, Yc)
        ...
        for b in angle_bins:
            ...
            plt.xlim([30, 190])
        plt.grid()
        plt.savefig('_'.join(['plots/angles_vs_errors',ss,array,'.png']))
        plt.close()
        plt.figure(figsize=(6,6))
        for b in distance_bins:
            ...
            plt.xlim([0.02, 0.09])
        plt.grid()
        plt.savefig('_'.join(['plots/distances_vs_errors',ss,array,'.png']))
        plt.close()
