...
@tf.function()
def forward_pass(X,Xc,Yc,all_vars,N,H,W,chans_in,ch1,droprate,mixrate):
    ...
    # combine filters
    F1d = (mixrate)*F + (1-mixrate)*tf.repeat(F1,chans_in,axis=2)
    F23d = (mixrate)*Fo + (1-mixrate)*F23
                    ...
                    for chans_in in all_chans_in:
                        ...
                        if epoch > 0 and patience_count < patience:
                            with tf.GradientTape() as g:
                                out= forward_pass(X,Xc,Yc,all_vars,N,H,W,chans_in,ch1,droprate,mixrate)
                                error = mse(out,Y)
