...
patience = 50
droprate = 0.0
optimizer = tf.optimizers.Adam(learning_rate=lr,beta_1=0.85) 

def extract_features(X, fs, datapoint_dur, nfft = 4096, hop_length=512):

    X = np.array([[np.abs(librosa.stft(x[i:i+int(datapoint_dur*fs)], n_fft=nfft, hop_length=hop_length))**2 for i in range(0,int(datapoint_dur*fs)*((len(x)+244)//int(datapoint_dur*fs)),int(datapoint_dur*fs))] for x in X]).transpose(1,2,3,0)
...
for array in marco_data.keys():
    for ss in ['single_sources0deg']: 
        ...
        marco_data[array][ss] = X[range(10,len(X),40)]
...
def forward_pass(X,Xc,Yc,all_vars,N,H,W,chans_in,ch1,droprate):
    ...
    F = tf.transpose(tf.reshape(sh3, (chans_in,K,K,ch1)), perm=(1,2,0,3))
    ...
    Fo = tf.transpose(tf.reshape(Fo, (1,K,K,ch1)), perm=(1,2,3,0))
    ...
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b5)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F6,1,padding='SAME')
    out = tf.nn.bias_add(out,b6)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out3 = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b7)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F8,1,padding='SAME')
    out = tf.nn.bias_add(out,b8)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out4 = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b9)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F10,1,padding='SAME')
    out = tf.nn.bias_add(out,b10)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b12)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F13,1,padding='SAME')
    out = tf.nn.bias_add(out,b13)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b15)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F16,1,padding='SAME')
    out = tf.nn.bias_add(out,b16)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b18)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F19,1,padding='SAME')
    out = tf.nn.bias_add(out,b19)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    ...
    out = tf.nn.bias_add(out,b21)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
    out = tf.nn.conv2d(out,F22,1,padding='SAME')
    out = tf.nn.bias_add(out,b22)
    mu, sigma = tf.nn.moments(out,axes=0)
    out = tf.nn.batch_normalization(out,mu,sigma,offset=None,scale=None,variance_epsilon=1e-3)
    out = tf.nn.relu(out)
